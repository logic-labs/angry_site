...
def register():
    if request.method == 'POST':
        ...
        elif db_session.query(User).filter(User.name==username).first() is not None:
            error = 'User {} is already registered.'.format(username)
        ...
        if error is None:
            ...
            # )
            # db.commit()
            user = User(name=username, passwd=generate_password_hash(password))
            db_session.add(user)
            db_session.commit()
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        # ).fetchone()
        usr = db_session.query(User).filter(User.id==user_id).first()
        g.user = {
            "id": usr.id,
            "username": usr.name,
            "password": usr.passwd
